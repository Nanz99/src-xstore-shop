...
import CreateColorForm from './color.create.page'
import {
  checkSyncAllProduct,
  syncAllProduct,
  syncProduct,
...
import { getAllColor, removeColor } from '@/services/product.service'
import EditColor from './color.edit.page'
...
const ColorList: React.FC<ProductProps> = props => {
  const {
    dispatch,
    listProducts,
    totalService,
  ...
  const deleteColor = async (id) => {
    const { success } = await removeColor(id)
    if (success) {
  ...
  const columns: ColumnProps<ProductItem>[] = [
    ...
    //Name column
    {
      title: formatMessage({ id: 'menu.Properties.Color' }),
      key: 'name',
      align: 'center',
      render: listColor => {
        return (
      ...
      render(listColor) {
        return (
          ...
          <div style={{
            ...
            <p style={{ marginTop: 12, width: "120px" }}>{listColor?.description}</p>
          </div>
        ...
        return (
          <div>
            <Paragraph>
              ...
              <Tag color="orange">{formatDate(listColor.createdAt) || ''}</Tag>
            </Paragraph>
            <Paragraph>
              {formatMessage({ id: 'order.detailUpdateAt' })}:{' '}
              <Tag color="geekblue">{formatDate(listColor.updatedAt) || ''}</Tag>
            </Paragraph>
          </div>
        )
      },
        ...
        return (
          <div>
            <Button
              ...
              onClick={() => {
                handleShowEditForm(listColor?.id)
              }}
            />
            <Button
              type="primary" shape="circle" icon={<DeleteOutlined />} danger 
              onClick={() => handleRemoveColor(listColor?.id)}
            />
          </div>
        )
      },
    ...
    <PageHeaderWrapper>
      ...
      <Card>
        ...
        <Modal
          ...
          title={formatMessage({ id: 'color.addNew' })}
          centered={true}
        ...
        <Modal
          ...
          title={formatMessage({ id: 'color.edit' })}
          centered={true}
        ...
        <Modal title="Basic Modal" visible={visibleDelete} onOk={() => deleteColor(idColor)} onCancel={handleHideDelete}>
