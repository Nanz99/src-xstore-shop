...
const CreateStockin = props => {
  ...
  const getValueForm = () => {
    if (exportFormValue) {
      ...
      tempValue.stockinProducts = exportFormValue.orderProducts.filter(
        item => item.remain !== 0,
      )
  ...
  const handleCreate = async res => {
    handleChangeOrder(ORDER_STATUS.DONE)
    form.resetFields()
    handleClose()
    ...
    //   })
    //   setStatusComboOrder(ORDER_STATUS.DONE)
    //   form.resetFields()
    //   handleClose()
                    ...
                    {fields.map((field, index) => (
                      <Space key={field.key} align="start">
                        ...
                        >
                          ...
                          <Select disabled={false} showSearch size="small">
