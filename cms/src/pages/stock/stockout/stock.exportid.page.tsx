...
const CreateStockin = props => {
  const { id, exportFormValue, listProduct, handleCancel } = props
  ...
  const handleChangeStatusOrder = async (id, value) => {
  ...
  const handleCreate = async res => {
    ...
    if (success === true) {
      ...
      const tempArray = res.orderProducts.filter(item => item.remain > 0)
      if (tempArray === 0) {
        handleChangeStatusOrder(exportFormValue.id, ORDER_STATUS.DONE)
      }
      form.resetFields()
      handleClose()
    } else {
                    ...
                    {fields.map((field, index) => (
                      <Space key={field.key} align="start">
                        ...
                        <Form.Item
                          ...
                          fieldKey={[field.fieldKey, 'remain']}
                          rules={[
                            {
                              required: true,
                              message: formatMessage({
