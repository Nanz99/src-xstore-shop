...
const CreateStockin = props => {
  ...
  const getValueForm = () => {
    ...
    if (exportFormValue) {
      ...
      tempValue.stockinProducts = exportFormValue.stockinProducts?.map(item => ({...item, productName: item.product.product.name, size: item?.product?.size?.description,
      color: item?.product?.color?.description
      }))

      // set quantity item to calculate every product
      const tempQuantity: any = [...quantity]
      for (let i = 0; i < exportFormValue.stockinProducts?.length; i++) {
        tempQuantity[i] = exportFormValue.stockinProducts[i].quantity
      }
  ...
  const handleCreate = async res => {
    ...
    const { success } = await putStockinService(exportFormValue.id, data)
    if (success === true) {
      ...
      form.resetFields()
      handleClose()
      history.go(`/stock/stockin/${exportFormValue.id}`)
    } else {
      notification.success({
        icon: <CheckCircleOutlined style={{ color: 'red' }} />,
        message: formatMessage({ id: 'stockin.editFail' }),
      })
      form.resetFields()
      handleClose()
    }
